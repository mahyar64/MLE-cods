    ...
    public:
        ...
        */
                           ...
                           const std::vector<std::vector<fann_type> > & output)
        {
            ...
            struct fann_train_data *data =
                (struct fann_train_data *)malloc(sizeof(struct fann_train_data));
            data->input = (fann_type **)calloc(input.size(), sizeof(fann_type *));
            data->output = (fann_type **)calloc(output.size(), sizeof(fann_type *));

            data->num_data = input.size();
            data->num_input = input[0].size();
            data->num_output = output[0].size();

            fann_type *data_input = (fann_type *)calloc(data->num_input*data->num_data, sizeof(fann_type));
            fann_type *data_output = (fann_type *)calloc(data->num_output*data->num_data, sizeof(fann_type));

            for (unsigned int i = 0; i < data->num_data; ++i)
            {
                data->input[i] = data_input;
                data_input += data->num_input;
                memcpy(data->input[i], &input[i][0], sizeof(fann_type)*data->num_input);
                data->output[i] = data_output;
                data_output += data->num_output;
                memcpy(data->output[i], &output[i][0], sizeof(fann_type)*data->num_output);
            }
            set_train_data(data);
        }
    ...
    */
	...
	/* Method: get_sarprop_temperature
        ...
        */ 
        ...
        //Accessor
        operator struct fann*() {return ann;}
